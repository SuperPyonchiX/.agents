...
### Obsidian の無い PC（会社PCなど）

第二の脳は普通のフォルダで代用する。**中身はその PC の外に出さない**（git で履歴を取るのはよいが remote は追加しない）。

1. 上の手順1でこのリポジトリを clone する。Codex はユーザー単位のスキルを `~/.agents/skills` から読む（導入先で一覧に出るか確認する）。シンボリックリンクを張れず Claude Code も使う場合は、`skills/` を `~/.claude/skills/` へコピーする
2. ワークスペースを作る（例 `~/brain`）。フォルダは `00_受信箱` `04_ノウハウ` `リサーチ`。直下に下の雛形で `AGENTS.md` を、`@AGENTS.md` の1行だけで `CLAUDE.md` を置く
3. `~/.codex/AGENTS.md` に、自宅と同じ「私について」「応答スタイル」「学習した指示」の節と、ワークスペースの場所を書く。Claude Code も使うなら `~/.claude/CLAUDE.md` は `@~/.codex/AGENTS.md` の1行にする。指示の正本を1本にするため

ワークスペースの `AGENTS.md` の雛形:

```markdown
# AGENTS.md

このフォルダは私の「第二の脳」。調べ物の結果と、試して分かったことを Markdown で溜める。

## 外に出さない

- ここの内容を社外のサービスへ送らない（Web 検索の語句、NotebookLM などへのアップロード、外部の AI への貼り付けを含む）。必要なときは先に確認する
- git の remote を追加しない
...
| フォルダ | 入れるもの |
|---|---|
| `00_受信箱` | 分類に迷うもの、思いつきのメモ。判断がつかないときの既定の置き場 |
| `04_ノウハウ` | 自分で試して分かったこと（ツールの設定、仕事の手順、ハマりどころ）。1テーマ1ファイル、既存があれば追記 |
| `リサーチ` | 調べ物の結果。`YYYY-MM-DD_テーマ名.md`。結論・根拠・出典の3点を必ず書き、保存先のパスを伝える |

- 深い階層は作らない。迷ったら `00_受信箱`
- 書く前に関連ファイルを検索し、あれば本文でファイル名に触れる
```
...
| 症状 | 疑うところ | 直し方 |
| --- | --- | --- |
| 発火しない | `description` | 「いつ使うか」を具体的な依頼文言で追記する。「明示的な言及がなくても使う」の類の定型句は足しても効かない |
