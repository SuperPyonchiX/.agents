...
description: Web 画面の見た目の指摘（見切れ・色・位置・余白・崩れ）を、指摘1件ごとに対象要素を特定→修正→同じ条件で再撮影→修正前後の突き合わせ→ユーザー確認で閉じる工程スキル。playwright-cli で撮影と要素特定を行い、指摘は台帳で管理して未クローズ残ゼロで完了とする。「見た目を直して」「添付図のように崩れている」「見切れている」「色が見にくい」「レイアウトを確認して」「スクショで確認して」で使う。新しい画面の配色・レイアウトを決めるのは ui-ux-pro-max、エージェントが撮影できない実機での確認は manual-test-sheet。
metadata:
