---
name: ui-ux-pro-max
description: Web・モバイル・デスクトップ（WPF・WinUI を含む22スタック）の UI を作る前に、画面の種類に合うスタイル・配色・フォント・UX ガイドライン・スタック別の実装指針をローカルのデータから検索し、デザインシステムとして返すスキル。「管理画面のデザインを決めて」「配色とフォントを選んで」「このダイアログを見やすくしたい」「UI をレビューして」で使う。作った画面の見切れや崩れを直すのは ui-visual-verify。
license: MIT
metadata:
  web-description: Web・モバイル・デスクトップの UI を設計・実装・レビューするときの判断材料を、スタイル・配色・フォント・UX ガイドライン・22スタックの実装指針から検索して返す。新しい画面を作る、配色やレイアウトを決める、UI を見直すときに使う（外部取り込み・MIT）。
---
