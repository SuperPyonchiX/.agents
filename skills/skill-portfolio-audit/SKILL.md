...
| 依頼のきっかけ | モード | 進め方 |
|---|---|---|
| スキルが増えた、意図しないスキルが発火する、README とずれている | 横断検査 | 以下の A0〜A4 |
| 使うモデルが変わった、スキルの中身が古くなっていないか見たい | 本文監査 | `references/body-audit.md` の B0〜B4 |
| 両方 | 横断検査を先に終えてから本文監査 | description を直した後に本文を見る |
| どれとも決めかねる | — | どちらをやるか尋ねる |
...
- **横断検査で全スキルの SKILL.md 本文を読むこと。** 判定に必要なのは description であり、本文が要ると感じた時点でそれ自体が発見である。本文監査でも、読むのは `scan_bodies.py` が挙げた行の前後だけ。
- **実行できなかった検査を「問題なし」として報告すること。**
- **上限を超えた候補を黙って切り捨てること。** 未判定として件数と一覧を残す。
- **検査のついでにスキルの中身を書き直すこと。** このスキルは置き場の健全性を見るものであって、個々のスキルの設計は `workflow-skill-architect` の担当。本文監査で直すのも台帳で「修正」とした行だけ。
...
| `references/body-audit.md` | 本文監査を選んだとき。B0〜B4 の手順・観点表・台帳の形式 |
...
| `scripts/scan_bodies.py` | B1・B4。本文監査の候補行の抽出と、是正後の再確認 |
...
`scan_bodies.py` は各スキルの SKILL.md と `references/` 配下の .md を行単位で走査し、モデル名・確認語の無い危険操作・特定エージェント専用のツール名・時点依存の記述・強調語の密度を拾う（kind の意味は `references/body-audit.md` の B1）。`--skills` で対象を絞れる。終了コードは 0（走査した。候補の有無は問わない）/ 2（引数の指定ミス・存在しないスキル名）。依存は標準ライブラリのみ。
