...
description: CodeSonar / Helix QAC などの静的解析指摘を、修正・逸脱・誤検知に仕分ける工程スキル。社内様式の Excel（誤検知・逸脱申請書）から指摘を読み、フィンガープリントで同種を束ねて代表1件で判断し、過去の判定を判定DBから再適用したうえで、申請書への書き戻しと逸脱記録書の生成まで行う。「静的解析の指摘を仕分けたい」「CodeSonar の結果をトリアージして」「QAC の指摘を分類して」「逸脱申請書を作って」で使う。コードを読んで指摘を新たに出すのは cpp14-code-review。
metadata:
