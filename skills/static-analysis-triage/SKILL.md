...
description: CodeSonar / Helix QAC などの静的解析ツールが出した大量の指摘を、修正・逸脱・誤検知に仕分ける工程スキル。社内様式の Excel（誤検知・逸脱申請書）から指摘を読み取り、ルールとコードパターンのフィンガープリントで同種の指摘を束ねて代表1件で判断し、過去の判定を判定DBから自動再適用したうえで、申請書への書き戻しと逸脱記録書の生成までを行う。誤検知には解析限界の分類コード、逸脱には理由コードと根拠を必須にし、未判定ゼロを機械判定するまで完了としない。「静的解析の指摘が大量に出たので仕分けたい」「CodeSonar の結果をトリアージして」「QAC の指摘を分類して」「これは誤検知か逸脱か判断して」「逸脱申請書を作って」「逸脱記録書をまとめて」「指摘の仕分けを手伝って」といった話が出たら、明示的に「スキル」という言及がなくても必ずこのスキルを使うこと。コードを読んで指摘を新たに出す工程は cpp14-code-review の担当なので、そちらを使う。既に出ている指摘を仕分けるのがこちら。Use this when triaging a large volume of existing static-analysis findings from CodeSonar or Helix QAC into fix / deviation / false-positive, driven by an Excel deviation-application form. To generate findings by reviewing code, use cpp14-code-review instead.
---
