...
description: 組込み C++14（AUTOSAR C++14 / CERT C++ 準拠）で、すでに動いているコードが期待どおり動かないときの原因究明を行う工程スキル。現象を再現条件まで確定させ、直近の変更点と照合して容疑範囲を絞り、仮説を確度と切り分けコストで並べて1件ずつ検証し、根本原因を確定してから恒久対策と再発防止テストを入れ、同じ原因パターンの水平展開まで行う。「このバグの原因を調べて」「動かないので原因究明したい」「なぜこの値になるのか切り分けたい」「たまに落ちるので調べて」「デバッグを手伝って」「不具合報告が来たので解析して」「リグレッションの原因を特定して」「実機だけで症状が出る」といった話が C / C++ の不具合について出たら、明示的に「スキル」という言及がなくても必ずこのスキルを使うこと。コードを読んで規約違反を指摘する工程は cpp14-code-review、静的解析ツールが出した指摘の仕分けは static-analysis-triage、設計から新規に実装する工程は nextdesign-cpp14-implementation の担当なので、そちらを使う。動いているコードが期待どおり動かない理由を突き止めるのがこちら。
metadata:
