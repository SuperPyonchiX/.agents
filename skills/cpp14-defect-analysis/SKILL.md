...
description: 組込み C++14 で、動いているコードが期待どおり動かないときの原因究明を行う工程スキル。現象を再現条件まで確定させ、直近の変更と照合して容疑範囲を絞り、仮説を確度と切り分けコストで並べて1件ずつ検証し、根本原因の確定・恒久対策・再発防止テスト・同種パターンの水平展開まで行う。「このバグの原因を調べて」「なぜこの値になるのか切り分けたい」「たまに落ちる」「実機だけで症状が出る」「デバッグを手伝って」で使う。規約違反の指摘は cpp14-code-review、究明後の報告書化は markdown-doc（不具合報告書）。
metadata:
